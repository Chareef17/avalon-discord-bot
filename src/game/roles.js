...
// ชุดบทบาทแนะนำตามจำนวนผู้เล่น
// สัดส่วนดี/ร้าย: 5: 3/2, 6: 4/2, 7: 4/3, 8: 5/3, 9: 6/3, 10: 6/4
...
function buildRoleDescriptionForPlayer(player, allPlayers) {
  ...
  if (role.key === 'MERLIN') {
    const list =
      visibleEvilsForMerlin.length > 0
  ...
  // ตกหล่นกรณีพิเศษให้เป็นฝ่ายร้ายทั่วไป
  return [
...
// ฟังก์ชันหลัก: สุ่มบทบาทให้ผู้เล่น และคืนข้อมูลที่ใช้ส่ง DM
// players: [{ id, name, role? }]
export function assignRolesAndGetInfos(players) {
  ...
  const infos = shuffledPlayers.map((p) => ({
    id: p.id,
